...
import { currentUser as mockCurrentUser, mockUsers } from '../../data/mock/users';

// Dados mockados do usuário atual - AGORA IMPORTADO
...
// Dados mockados de amigos (agora usando mockUsers)
const mockFriends: Friend[] = mockUsers.slice(0, 6).map((user, index) => ({
  id: `friend-${index + 1}`,
  user: user,
  friendshipDate: `2024-01-${10 + index}`,
  mutualFriends: Math.floor(Math.random() * 5) + 1,
  commonTags: currentUser.tags.filter(tag => user.tags.includes(tag)).slice(0, 3)
}));

// Dados mockados de sugestões de amigos
const suggestedFriends: User[] = mockUsers.slice(6); // Os últimos 2 usuários
